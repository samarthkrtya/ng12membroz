...
@Component({
  selector: 'app-appointment-resource-lists',
  templateUrl: './appointment-resource-lists.component.html',
  styles: [
  ],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => AppointmentResourceListsComponent),
      multi: true
    }
  ]
})
export class AppointmentResourceListsComponent  implements OnInit, ControlValueAccessor {
  ...
  ngOnInit() {
    ...
    if(this.dbvalue) {
      var roomObj = this.options.find(p=>p._id == this.dbvalue);
      if(roomObj) {
        this.myControl.setValue(roomObj);
      }
    }
