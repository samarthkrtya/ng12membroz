...
export class DocumentCreateFolderComponent extends BaseLiteComponemntComponent implements OnInit {
  ...
  @Output() folderSubmitData: EventEmitter<any> = new EventEmitter<any>();
  ...
  onSubmitFolder(value: any, isValid: boolean) {
    ...
    if (!isValid) {
      ...
      postData['title'] = value.foldername;
      ...
      return this._commonService
        ...
        .then( (data: any) => {
          if(data) {
            let postData = {}
            this.folderSubmitData.emit(postData);
            return;
          }
      }, (error) =>{
        console.error(error);
