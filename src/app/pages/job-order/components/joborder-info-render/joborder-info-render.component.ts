...
@Component({
  selector: 'app-joborder-info-render',
  templateUrl: './joborder-info-render.component.html',
  styles: [
  ]
})
export class JoborderInfoRenderComponent extends BaseComponemntComponent implements BaseComponemntInterface, OnInit {
  ...
  filteredadvisoroptions: Observable<string[]>;
  allAdvisorLists: any[] = [];
  advisorisLoadingBox: boolean = false;
  ...
  constructor(
    ...
    this._route.params.forEach((params) => {
      ...
      this.pagename = 'app-joborder-info-render';
    });
  ...
  async ngOnInit() {
    ...
    try {
      ...
      if(this.custId){
        await this.getCustomerDetails(this.custId);
      }
      ...
      if (this.bindid) {
        ...
        await this.getOrderById(this.bindid)
      }
      ...
      this.filteredadvisoroptions = this.advisorid.valueChanges
        .pipe(
          startWith(''),
          map(option => typeof option === 'string' ? option : option.fullname),
          map(option => option ? this._advisorfilter(option) : this.allAdvisorLists.slice())
        );
  ...
  async getCustomerDetails(id: any) {
    ...
    if (assetObj) {
      assetObj.src = assetObj.profilepic ? assetObj.profilepic : '../../assets/img/default-avatar.png'
  ...
  async initializeVariables() {
    ...
    this.form.controls['dueby'].setValue(new Date());
    ...
    return this._commonService
      ...
      .then((data: any) => {
        ...
        if (data && data[0]) {
          ...
          this.joborderDetail = {};
          this.joborderDetail = data[0];

          this.form.controls['_id'].setValue(this.joborderDetail._id);

          if (this.joborderDetail && this.joborderDetail.assetid) {
            
            var assetObj = this.allAssetLists.find(p=>p._id == this.joborderDetail.assetid._id);
            if(assetObj) {
              this.assetid.setValue(assetObj);
              this.form.get("assetid").setValue(this.assetid);
            }
            this.customerWiseInspection = this.allInspectionLists.filter(p => p.assetid == this.joborderDetail.assetid._id);
          }

          this.form.controls['date'].setValue(new Date(this.joborderDetail.date));
          this.form.controls['dueby'].setValue(new Date(this.joborderDetail.dueby));

          if (this.joborderDetail && this.joborderDetail.advisorid) {
            this.advisorid.setValue(this.joborderDetail.advisorid);
          }

          if (this.joborderDetail && this.joborderDetail.property && this.joborderDetail.property.inspectionid) {
            var inspectionObj = this.customerWiseInspection.find(p => p._id == this.joborderDetail.property.inspectionid);
            if (inspectionObj) {
              this.inspectionid.setValue(inspectionObj);
              this.form.get("inspectionid").setValue(this.inspectionid);
            }
  ...
  async onDelete() {
    ...
    if (!this.bindid) {
      swal.fire({
        ...
        if (result) {
          this._router.navigate(['pages/inspection-module/job-order']);
        }
      });
    } 
    else {
          ...
          await this._commonService
            ...
            .then(data => {
              if (data) {
                ...
                this._router.navigate(['pages/dynamic-list/list/joborder']);
                this.disableBtn = false;
              }
            }).catch((error) => {
              console.error(error);
  ...
  onSubmit(value: any, isValid: boolean) {
    ...
    if (!isValid) {
      ...
      if (value._id) {
        ...
        postData['items'] = this.joborderDetail.items;
        postData['services'] = this.joborderDetail.services;
        postData['prefix'] = this.joborderDetail.prefix;
        postData['amount'] = this.joborderDetail.amount;
        postData['totalamount'] = this.joborderDetail.totalamount;
        postData['taxamount'] = this.joborderDetail.taxamount;
        postData['taxdetail'] = this.joborderDetail.taxdetail;
        postData['discount'] = this.joborderDetail.discount;

        var url = "joborders/" + value._id
        var method = "PUT";
        ...
        return this._commonService
          ...
          .then((data: any) => {
            if (data) {
              ...
              super.showNotification("top", "right", "Job Order has been Updated successfully !!", "success");
  ...
  preloadAdvisordata() {
    if (this.allAdvisorLists && this.allAdvisorLists.length == 0) {
      this.getAllAdvicer()
    }
  }

  handleEmptyAdvisorInput(event: any) {
    if (event.target.value === '') {
  ...
  displayAdvisorFn(user: any): string {
    return user && user.fullname ? user.fullname : '';
  }

  optionAdvisorSelected(option: any) {
    this.advisorid.setValue(option.value);
  }

  private _advisorfilter(value: string): string[] {
    let results;
    if (value) {
      results = this.allAdvisorLists
        .filter(option => {
          if (option.fullname) {
      ...
      results = this.allAdvisorLists.slice();
    }
    return results;
  }
