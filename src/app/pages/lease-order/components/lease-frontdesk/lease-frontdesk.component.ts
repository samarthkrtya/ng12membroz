...
@Component({
  selector: 'app-lease-frontdesk',
  templateUrl: './lease-frontdesk.component.html',
})

export class LeaseFrontdeskComponent extends BaseLiteComponemntComponent implements OnInit, OnDestroy {
  ...
  periodtype: string[] = ['Hourly', 'Daily', 'Monthly', 'Yearly'];
  ...
  constructor(
    ...
    if (this.type) {
      this.selectedIndexes = this.type == 'product' ? 1 : this.type == 'service' ? 2 : 0;
    ...
    this.serviceForm = this.fb.group({
      ...
      'period': [, Validators.compose([Validators.required, OnlyPositiveNumberValidator.insertonlypositivenumber])],
      'title': [''],
      'bookingdate': [],
      'cost': [0],
      // 'taxes': [],
  ...
  async onStatusChanges() {
    if (this.leaseorderList2 && this.leaseorderList2.length > 0) {
      this.isLoading['appointment'] = true;
      var statusExists: boolean = false;
      this.leaseorderList = [];
      this.leaseorderList2.forEach(element => {
        ...
        if (statusExists) {
          this.leaseorderList.push(element);
        }
      });
      this.isLoading['appointment'] = false;
    }
  ...
  async getJobOrder() {
    ...
    this.leaseorderList = [];
    this.leaseorderList2 = [];

    let api = "leaseorders/filter";
    let method = "POST";
    this.isLoading['appointment'] = true;
    await this._commonService
      .commonServiceByUrlMethodDataAsync(api, method, postData)
      .then((data: any) => {
        this.leaseorderList = data;
        this.leaseorderList.map(a => a.shrname = a.customerid && a.customerid.fullname ? a.customerid.fullname.match(/\b(\w)/g).join('') : '--');
        this.leaseorderList.map((a) =>{
          a.selected = false
          a.newStatus = this.statusList.find(s => s.code == a.status).value;
         });

        this.leaseorderList2 = this.leaseorderList;
        this.isLoading['appointment'] = false;

      });
  }

  async LeaseById(id: any) {
    this.isLoadingCart = true;
    this.servicecartList = [];
    this.bindid = id;
    this.cartItemList = [];

    this.leaseorderList.map(val => val.selected = false);
    this.billItemList.map(val => val.quantity = 0);
    this.serviceList.map(val => val.selected = false);

    var leaseorder = this.leaseorderList.find(a => a._id == id);
    leaseorder.selected = true;
    if (this.customerList.length > 0 && leaseorder['customerid']) {
      this.searchMember = this.customerList.find(a => a._id == leaseorder['customerid']['_id'])
    }

    if (leaseorder['billid']) {

      this._billModel._id = '';
      this._billModel.status = '';

      let postData = {};
      postData['formname'] = "bill-lease";
      postData['search'] = [];
      postData["search"].push({ "searchfield": "billid", "searchvalue": leaseorder['billid']['_id'], "criteria": "eq", "datatype": 'ObjectId' });
      ...
      if (billPaymentRes && billPaymentRes.length > 0) {
        ...
        billRes = await this._billService.AsyncGetById(leaseorder['billid']['_id']) as any;                        // billRes
      }
    ...
    if (leaseorder.assets && leaseorder.assets.length > 0) {
      leaseorder.assets.forEach(tsk => {
        var service = this.serviceList.find(a => a._id == tsk['refid']['_id']);
        service['selected'] = true;
        this.servicecartList.push({
          'refid': tsk.refid._id,
          'discount': tsk.discount,
          'bookingdate': tsk.bookingdate,
          'periodtype': tsk.periodtype,
          'period': tsk.period,
          'cost': tsk.cost,
          'status': tsk.status,
          // 'taxes': tsk.taxes,
    ...
    if (leaseorder.items && leaseorder.items.length > 0) {
      leaseorder['items'].forEach(itm => {
        ...
        if (billitem) {
          ...
          billitem['period'] = itm.period;
          billitem['periodtype'] = itm.periodtype;
  ...
  async getBillbyId(id: any) {
    try {
      ...
      var leaseorder = this.leaseorderList.find(a => a.billid && a.billid._id ? a.billid._id == this.billid : undefined);
      if (leaseorder) {
        leaseorder['selected'] = true;
        this.bindid = leaseorder._id;
      }
      setTimeout(() => {
        this.makeModel();
        this.GetBillDetail(this._billModel);
  ...
  async getBillPaymentbyId(id: any) {
    try {
      ...
      let postData = {};
      postData['formname'] = "billpaymentlease";
      postData['search'] = [];
  ...
  onClickService(refid: any) {
    ...
    if (this.selectedService.availability && this.selectedService.availability.days.length > 0 && this.selectedService.availability.days.includes(day)) {
      ...
      var cartItem = this.servicecartList.find(a => a.refid == this.selectedService._id);
      if (!cartItem) {
        this.serviceForm.reset();
        this.serviceForm.controls['bookingdate'].setValue(new Date());
        ...
        this.serviceForm.controls['cost'].disable();
        this.serviceForm.controls['periodtype'].setValue(cartItem.periodtype);
        this.serviceForm.controls['period'].setValue(cartItem.period);
        // this.serviceForm.controls['taxes'].setValue([]);
        // if (this.selectedService.taxes && this.selectedService.taxes.length > 0) {
        //   this.serviceForm.controls['taxes'].setValue(this.selectedService.taxes);
        // }
      ...
      super.showNotification("top", "right", `Asset not availble today !!`, "danger");
      return;
    }
  }
  ...
  async saveJO() {
    ...
    model['assets'] = [];
    model['assets'] = this.servicecartList;
    model['items'] = [];
    ...
    await this._commonService
      ...
      .then(async (data: any) => {
        if (data) {
          ...
          await this.getJobOrder();
          this.LeaseById(data._id);
          this.disableBtn = false;
          this.isLoadingCart = false;
          this.showNotification("top", "right", "Lease order updated !!", "success");
        }
      }).catch((e) => {
  ...
  async onRemoveService(refid: any, i: number) {
    var selectedService = this.serviceList.find(a => a._id == refid);
  ...
  redirecttoURL(event: any) {
    this._router.navigate([`/pages/sale-module/multiple-bill/bill-lease/` + event._id]);
  }
  ...
  onPrint(id: any) {
    this._router.navigate(['/pages/dynamic-preview-list/bill-lease/' + id]);
  }
  ...
  async onSaveBill() {
    ...
    this._billModel.type = 'leaseorder';
  ...
  clearData() {
    ...
    this.leaseorderList.map(a => a.selected = false);
