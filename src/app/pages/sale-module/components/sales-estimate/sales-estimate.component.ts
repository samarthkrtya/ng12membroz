...
export class SalesEstimateComponent extends BaseLiteComponemntComponent implements OnInit, OnDestroy {
  ...
  async onNewQutn() {  // save qutation here
    try {
      ...
      if (billRes && billRes['_id']) {
        ...
        this._router.navigate([`/pages/dynamic-preview-list/quotation/${billRes['_id']}`]);
        // this.bindid = billRes['_id'];
        // await this.getQutation();
        // var qn = this.quotationList.find(a => a._id == this.bindid);
        // if (qn) {
        //   qn.selected = true;
        // }
      }
  ...
  async onSaveQn() {
    ...
    model['type'] = this.type ? this.type : 'pos';
